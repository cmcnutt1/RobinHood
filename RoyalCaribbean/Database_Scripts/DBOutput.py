...
def init_db(ship_prefix):

    db = TinyDB('individual_ships/' + ship_prefix + '.json')
    return db


def print_individual_entry(ship_name):

    ship_prefix = ship_name.lower()

    db = init_db(ship_prefix)
